...
namespace winmd::reader
{
    inline uint32_t uncompress_unsigned(byte_view& cursor)
    {
        ...
        {
            impl::throw_invalid("Invalid compressed integer in blob");
        }
    ...
    private:
        static CallingConvention check_convention(byte_view& data)
        {
            ...
            {
                impl::throw_invalid("Invalid calling convention for field blob");
            }
            return conv;
        }
        CallingConvention m_calling_convention;
    ...
    private:
        static CallingConvention check_convention(byte_view& data)
        {
            ...
            {
                impl::throw_invalid("Invalid calling convention for property blob");
            }
            return conv;
        }
        CallingConvention m_calling_convention;
        ...
        , m_generic_arg_count(uncompress_unsigned(data))
    {
        ...
        {
            impl::throw_invalid("Generic type instantiation signatures must begin with either ELEMENT_TYPE_CLASS or ELEMENT_TYPE_VALUE");
        }
    ...
    inline TypeSig::value_type TypeSig::ParseType(database const& db, byte_view& data)
    {
        ...
        default:
            impl::throw_invalid("Unrecognized ELEMENT_TYPE encountered");
            break;
        }
    }
}
